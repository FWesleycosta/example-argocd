...
- **Destroy do sandbox (`2.6.0`)**: `destroySandbox: true` num run **manual** de `sandbox/*`
  roteia para `stages/destroy-sandbox.yaml` (stage único, Environment `sdx`): delete do
  namespace K8s → `steps/terraform-destroy.yaml` — verifica o state (**nunca** cria bucket),
  preserva `aws_api_gateway_account` (`state rm`; singleton da conta compartilhada com dev),
  `plan -destroy` com relatório na Summary + artefato `sandbox-destroy` **antes** do apply,
