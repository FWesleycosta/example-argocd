...
steps:
  - script: |
      ...
      def yq_safe(expr, default=''):
          try:
              result = subprocess.check_output(['yq', 'e', expr, config_path], stderr=subprocess.DEVNULL).decode().strip()
              if result in ('null', ''):
                  return default
              return result
          except Exception:
      ...
      for key in handlers:
          ...
          if len(full_name) > MAX_LEN:
              print(f"##[error] Nome da função excede {MAX_LEN} chars: '{full_name}' ({len(full_name)} chars)")
              print(f"##[error] Adicione 'function_name_prefix' no config YAML com um prefixo mais curto.")
              print(f"##[error] Máximo para o prefixo com handler '{key}': {MAX_LEN - len(key) - 1} chars")
              sys.exit(1)
          else:
