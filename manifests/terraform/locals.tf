locals {
  ...
  # Mesma convenção de nome já usada em sns_topics e sqs_queues.
  lambda_functions = {
    for f in var.lambda_functions : f.function_name => merge(f, {
      function_name = "lambda-${var.environment}-${data.aws_region.current.name}-${f.function_name}"
    })
  }

  # A role de execução só ganha permissão de ENI / X-Ray se alguma função
  # realmente precisar — evita conceder para todo mundo por precaução.
  lambda_needs_vpc = length([
    for f in local.lambda_functions : f
    if length(coalesce(f.subnet_ids, [])) > 0
  ]) > 0

  lambda_needs_xray = length([
    for f in local.lambda_functions : f
    if f.tracing_config == "Active"
  ]) > 0
}